...
const getData = async (req, res) => {
  try {
    ...
    const data = await WhyChooseUs.find(query).skip(skip).limit(limit);
    const totalDataCount = await WhyChooseUs.countDocuments(query);
...
const updateData = async (req, res) => {
  try {
    ...
    await WhyChooseUs.findByIdAndUpdate(id, updatedFields, {
      new: true,
    });

...
import OurJourneyItem from "../models/OurJourneyItem.model.js";
const getDataFrontend = async (req, res) => {
  try {
    const data = await OurJourneyItem.find();
...
const getData = async (req, res) => {
  try {
    ...
    const data = await OurJourneyItem.find(query).skip(skip).limit(limit);
    const totalDataCount = await OurJourneyItem.countDocuments(query);
...
const addData = async (req, res) => {
  try {
    ...
    const newData = new OurJourneyItem({
      title,
...
const updateData = async (req, res) => {
  try {
    ...
    const data = await OurJourneyItem.findById(id);
    ...
    await OurJourneyItem.findByIdAndUpdate(id, updatedFields, {
      new: true,
    });
...
const deleteData = async (req, res) => {
  try {
    ...
    await OurJourneyItem.findByIdAndDelete(id);
